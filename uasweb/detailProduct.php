...
if(isset($_POST['buyNow'])){
    $sql = mysqli_query($conn, "SELECT stok_gtr FROM produk WHERE id_gtr = '$id'");
    while($i = mysqli_fetch_array($sql)){
        $sqlKurang = mysqli_query($conn, "UPDATE produk SET stok_gtr = stok_gtr - 1 WHERE id_gtr = '$id'");
...
<div class="buyContainer">
    <form action="" method="post">
            <div class="tampilDetail">
                <label for=""></label>
                <?php
                $sql2 = "SELECT nama_gtr, harga_gtr, gambar FROM produk WHERE id_gtr = '$id'";
                $result = $conn->query($sql2);  
                if(mysqli_num_rows($result) > 0){
                    while($row = mysqli_fetch_assoc($result)){
                        ?>
                        <img src="image/<?php echo $row['gambar'] ?>" alt="Produk" style="max-width: 100px;height: auto;"><br>
                        <?php
                        echo $row['nama_gtr'], "<br>";
                        echo "Rp.". $row['harga_gtr'];
                    }
                }
                ?>
            </div>
